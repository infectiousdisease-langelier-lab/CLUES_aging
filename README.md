...
- `BMI_metadata.csv`: Metadata for the CLUES bulk cohort with BMI information.

- `hydro_meta.csv`: Metadata for the CLUES bulk cohort with hydroxychloroquine dosage information.

- `dxage_meta.csv`: Metadata for the CLUES bulk cohort with diagnosis age information.

- `all_IFN_pgml`: All proteomic data.
...
- `new_files/`: Folder containing the unnormalized pseudobulk counts for each cell type, after pseudobulk-level QC. Also contains metadata for 149 CLUES patients and 49 healthy controls.
...
- `BMI_upload.R`: Takes in `cleaned_CLUES_bulkcounts.csv` and `BMI_metadata.csv` as inputs. Performs DE and GSEA analysis controlling for BMI. 

- `hydro_upload.R`: Takes in `cleaned_CLUES_bulkcounts.csv` and `hydro_meta.csv` as inputs. Performs DE and GSEA analysis controlling for hydroxychloroquine treatment. 

- `DX_age.R`: Performs DE and GSEA analysis controlling for SLE diagnosis age. Takes in `cleaned_CLUES_bulkcounts.csv` and `dxage_meta.csv` inputs. 

- `score_dx.R`: Produces supplementary figure 10, calculating IFN scores controlling for diagnosis age. Takes in `cleaned_CLUES_bulkcounts.csv` and `dxage_meta.csv` as inputs.

- `proteomics.R`: Takes in `cleaned_CLUES_bulkcounts.csv` and `all_IFN_pgml.csv` as inputs. It calculates the slope and adjusted p value between the protein concentration and age. 
...
- `pseudobulk_analysis_NK.R`: Analyzes the NK subtype files in `new_files`. Performs QC on the pseudobulk file for each cell type, normalizes and conducts differential expression analysis using voom, computes GSEA results using fGSEA after calling topTable, and creates pathway plots for certain NK cell subtypes using the reactome pathways. 
...
- `supp8.R`: Takes in pseudobulk metadata from the `new_files` folder. Additionally reads in pseudobulk counts for each patient. Generates the pathway and volcano figures for supplementary figure 8 in the original manuscript.
...
- `score_methylation.R`: Performs methylation analysis which results in supplementary figure 10. Additionally, calculates interferon scores. 
