# Here I analyze the CLUES bulk data, analyzing the relationship between dxage
...
meta_complete <- read_csv("CLUES_Time1all.csv")
...
meta_complete <- meta_complete %>%
  ...
  mutate(SampleID = paste0(SubjectID, "DNA", "01"))

meta_complete <- meta_complete[meta_complete$SampleID %in% colnames(counts), ]

counts <- counts[,meta_complete$SampleID]
stopifnot(colnames(counts)==meta_complete$SampleID)

cleaned_metadata <- meta_complete

## Convert categorial variables into factors

cleaned_metadata$female <- as.factor(cleaned_metadata$female)
cleaned_metadata$RaceEth <- as.factor(cleaned_metadata$RaceEth)
cleaned_metadata <- cleaned_metadata %>%
  mutate(AGEDX_factor = factor(ifelse(AGEDX > 50, "1", "0")))


cleaned_metadata <- cleaned_metadata %>%
  dplyr::select(c(SampleID, AGEDX_factor, Age, RaceEth, female))

write.csv(x = cleaned_metadata, file = "dxage_meta.csv")
...
design <- model.matrix(~AGEDX_factor + female + RaceEth + Age,
...
## Extract genes
top_DE <- topTable(vfit, coef = "AGEDX_factor1", sort.by = "none",
...
pathway_plot <- ggplot(data = reactome.gsea2, aes(x=reorder(pathway,NES), y=NES)) +
  ...
  scale_fill_manual(name = "", values = c('TRUE' = "#e66101", 'FALSE' = "#5e3c99", '6' = 'white'), drop = FALSE,
                    breaks = c('TRUE', 'FALSE'), 
