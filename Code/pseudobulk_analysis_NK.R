# Here I look for genes DE with age based on single cell pseudobulk data for NK cells
...
# Create a directory where there are three flies
# NK_dim's pseudobulk, NK_bright's pseudobulk, and the metadata
...
## Input: bulk refers to the pseudobulk data (dataframe object) for a given cell type
## The shape should be (# of genes X # of samples)
...
NK_dim <- as.data.frame(pseudo_QC["annotated_CLUES_NK_dim_pseudobulk_count"])
NK_bright <- as.data.frame(pseudo_QC["annotated_CLUES_NK_bright_pseudobulk_count"])

# Clean pseudobulk data -----

## Inputs: current_df refers to the pseudobulk data (dataframe object) for a given cell type, name refers to the cell type name
## the shape of the dataframe should be (# of genes X # of samples) post QC
...
## Input: current_df refers to the pseudobulk data (dataframe object) for a given cell type. Its
## shape should be (# of genes X # of samples) post QC
...
## for SLE patients and controls, and the number of samples for both categories for that cell type
## the pseudobulk counts should be dataframes of the following dimension (# of genes X # of samples) post QC
## the metadata should be dataframes of the following dimension (# samples x # of metadata values)

## Output: A list containing the output of the DE analysis for the cell-specific pseudobulk
## data for SLE patients and controls. This output is an object of class MArrayLM.
...
## data for SLE patients and controls. The are objects of class MArrayLM.
...
## is the number of rows in Sig_control, which can be interpreted as the number of DE genes. nsig2cont
...
## Inputs: topTables is a list with the following items: top_control a dataframe with is the result of toptable on the DE analysis for the cell-specific
## pseudobulk data for controls. sig_control is the same dataframe but only with significantly DE genes. sig2_control
## is similar to sig_control, but with a less stringent threshold for DE genes 
##nsigcont is the number of rows in Sig_control, which can be interpreted as the number of DE genes. nsig2cont
## is the number of DE genes with a less stringent p.value threshold
## The same naming scheme applies to _lupus.
## all nsig values are integers

## name is the cell type name
## number_lup is the total number of lupus samples passing QC (integer)
## number_control is the total number of control samples passing QC (integer)
## nsigl and nsigc are the total number of DE genes for the pseudobulk of a given cell type (integers)
## where the threshold is p < 0.05
## sig2l and sig2c are the total number of DE genes for the pseudobulk of a given cell type (integers)
## where the threshold is p < 0.1

## Outputs: a list, "plots" with nsigl, nsigc, sig2l, sig2c, which are the same as inputs
## lupus.gsea, control.gsea which are the gsea results (list of dataframes) for the lupus and control groups
...
## The dataframe should be of shape (# of genes X # of samples)
...
NK_dim_plots <- complete(NK_dim, "NK_dim")


## To generate the NK bright data, remove the sex variable from the formula in lines 165 and 168

NK_bright_plots <- complete(NK_bright, "NK_bright")

NKd_CLUES.gsea <- NK_dim_plots[[5]]
NKd_RS.gsea <- NK_dim_plots[[6]]
NKd_CLUES.gsea$CT <- "NK_dim_CLUES"
NKd_RS.gsea$CT <- "NK_dim_RS"

NKb_CLUES.gsea <- NK_bright_plots[[5]]
NKb_RS.gsea <- NK_bright_plots[[6]]
NKb_CLUES.gsea$CT <- "NK_b_CLUES"
NKb_RS.gsea$CT <- "NK_b_RS"



binded <- rbind(NKd_CLUES.gsea, NKd_RS.gsea)

pathway <- ggplot(data = binded, aes(x=pathway, y=NES)) +
  ...
  guides(fill=guide_legend(nrow=1,byrow=TRUE)) + ylim(c(-3.0, 3.0)) + geom_hline(yintercept = 0) + facet_wrap(~ CT, ncol = 2)
